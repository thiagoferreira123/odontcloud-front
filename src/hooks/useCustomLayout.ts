...
import {
  ...
  menuResetUseSidebar,
} from '/src/layout/nav/main-menu/menuSlice';
import { settingsChangeColor, settingsChangeLayout, settingsChangeNavColor, settingsChangeRadius, settingsResetLayout } from '/src/settings/settingsSlice';
...
interface CustomLayoutOptions {
  ...
  useSidebar?: string | null;
  pinEnable?: string | null;
...
const useCustomLayout = ({
  ...
  } = useSelector((state:RootStateOrAny) => state.menu);
  const { layout: currentLayout, radius: currentRadius, color: currentColor, navColor: currentNavColor } = useSelector((state:RootStateOrAny) => state.settings);
  const variables = {
  ...
  useEffect(() => {
    ...
    if (fullpage !== null) {
      ...
      document.documentElement.setAttribute('data-fullpage', String(fullpage));
    }
