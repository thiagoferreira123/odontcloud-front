...
api.onGet(`${SERVICE_URL}/ferramentas/chat`).reply(200, chatData);
api.onPut(`${SERVICE_URL}/ferramentas/chat/addText`).reply((config) => {
  const requestData = JSON.parse(config.data);
  const { id, text } = requestData;
  const date = new Date();
  const selectedPatient = chatData.find((x) => x.id === id);
  ...
  // Add item
  chatData = [selectedPatient, ...chatData.filter((x) => x.id !== id)];
  return [200, chatData];
});
api.onPut(`${SERVICE_URL}/ferramentas/chat/addAttachments`).reply((config) => {
  const requestData = JSON.parse(config.data);
...
api.onPut(`${SERVICE_URL}/ferramentas/chat/read`).reply((config) => {
  const requestData = JSON.parse(config.data);
  const { id } = requestData;
  const selectedPatient = { ...chatData.find((x) => x.id === id), unread: 0 };
