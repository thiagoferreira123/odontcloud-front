...
api.onGet(`${SERVICE_URL}/ferramentas/tasks`).reply(200, tasksData);

api.onPut(`${SERVICE_URL}/ferramentas/tasks`).reply((config) => {
...
api.onPost(`${SERVICE_URL}/ferramentas/tasks`).reply((config) => {
  const requestData = JSON.parse(config.data);
  const { item } = requestData;
...
api.onDelete(`${SERVICE_URL}/ferramentas/tasks`).reply((config) => {
  const { id } = config;
  // Delete item
