...
api.onGet(`${SERVICE_URL}/ferramentas/events`).reply(200, eventsData);
api.onPost(`${SERVICE_URL}/ferramentas/events`).reply((config) => {
...
api.onPut(`${SERVICE_URL}/ferramentas/events`).reply((config) => {
  const requestData = JSON.parse(config.data);
  const { item } = requestData;
...
api.onDelete(`${SERVICE_URL}/ferramentas/events`).reply((config) => {
  const { id } = config;
  // Delete item
