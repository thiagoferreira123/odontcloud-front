...
api.onGet(`${SERVICE_URL}/ferramentas/contacts`).reply((config) => {
...
api.onPost(`${SERVICE_URL}/ferramentas/contacts`).reply((config) => {
...
api.onPut(`${SERVICE_URL}/ferramentas/contacts`).reply((config) => {
  const requestData = JSON.parse(config.data);
  const { item, sortBy, pageSize, pageIndex } = requestData;
  let dataList = [...contactsAppData];
  // Update item
...
api.onDelete(`${SERVICE_URL}/ferramentas/contacts`).reply((config) => {
