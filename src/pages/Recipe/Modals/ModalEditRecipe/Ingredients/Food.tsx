...
export default function Food(props: FoodProps) {
  const { updateSelectedRecipeFood, updateSelectedRecipeFoodMacros, removeSelectedRecipeFood } = useEditReciptStore();

  const handleChangeQuantity = async (e: React.ChangeEvent<HTMLInputElement>) => {
    const parsedValue = Number(e.target.value);

    updateSelectedRecipeFood({ ...props.food, quantidade: parsedValue });
    updateSelectedRecipeFoodMacros({ ...props.food, quantidade: parsedValue });
  };

  const removeFood = async () => {
    try {
      removeSelectedRecipeFood(props.food);
    } catch (error) {
