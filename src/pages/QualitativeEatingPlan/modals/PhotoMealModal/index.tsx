...
import { MealImage } from '/src/types/ImagemRefeicao';
...
const PhotoMealModal: React.FC = () => {
  const showModal = usePhotoMealModalStore((state) => state.showModal);
  const selectedMeal = usePhotoMealModalStore((state) => state.selectedMeal);
  ...
  const { getPhotos, closeModal, removePhoto, addPhoto } = usePhotoMealModalStore();
  ...
  const onSubmit = async (values: { description: string }) => {
    ...
    try {
      const newPhoto = await addPhoto(file, values, queryClient);

      if(newPhoto === false) return;
  ...
  const handleSelectImage = async (photo: MealImage) => {
    ...
    try {
      ...
      await api.patch('/plano-alimentar-qualitativo-refeicao/' + selectedMeal.id, payload);
  ...
  const photoResult = useQuery({ queryKey: ['photos'], queryFn: getPhotos, enabled: showModal });
  ...
  const handleRemovePhoto = async (photo: MealImage) => {
    ...
    await removePhoto(photo, queryClient);
  };
