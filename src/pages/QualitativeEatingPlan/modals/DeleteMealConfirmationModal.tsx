...
const DeleteMealConfirmationModal = () => {
  const showModal = useDeleteMealConfirmationModalStore((state) => state.showModal);

  const selectedMeal = useDeleteMealConfirmationModalStore((state) => state.selectedMeal);
  const { removeQualitativeEatingPlanMeal } = useQualitativeEatingPlanStore();
  ...
  const { hideModal } = useDeleteMealConfirmationModalStore();
  ...
  const onSubmit = async () => {
    ...
    try {
      ...
      const response = await removeQualitativeEatingPlanMeal(selectedMeal);

      if (response === false) throw new Error('Erro ao remover a refeição');
...
export default DeleteMealConfirmationModal;
