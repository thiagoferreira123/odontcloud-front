...
const ModalSendPDF = () => {
  ...
  const onSubmit = async () => {
    ...
    try {
      ...
      await api.post('/plano-alimentar-equivalente-email/' + planId, payload);
    ...
    <Modal show={showModalSendPDF} onHide={() => setShowModalSendPDF(false)} backdrop="static" size="lg" className="modal-close-out">
      ...
      <Modal.Body>
        <Accordion defaultActiveKey="0" flush>
          ...
          <Accordion.Item eventKey="replacementList">
            <Accordion.Header as="div">
              <Form.Check
                type="checkbox"
                id="basicCheckbox"
              ...
              {!selectedFoods.length ? ' - ⚠️ Esse material não foi elaborado, e por tanto, não possui PDF.' : ''}
            </Accordion.Header>
          </Accordion.Item>
          ...
          <Accordion.Item eventKey="orientations">
            <Accordion.Header as="div">
              <Form.Check
                ...
                disabled={!orientations.filter((item) => typeof item.id == 'number').length}
              />{' '}
              Orientações gerais
              {!orientations.filter((item) => typeof item.id == 'number').length ? ' - ⚠️ Esse material não foi elaborado, e por tanto, não possui PDF.' : ''}
            </Accordion.Header>
          </Accordion.Item>
          ...
          <Accordion.Item eventKey="comments">
            <Accordion.Header as="div">
              <Form.Check
                ...
                disabled={!meals.filter((meal) => meal.comentario).length}
              />{' '}
              Comentários das refeições
              {!meals.filter((meal) => meal.comentario).length ? ' - ⚠️ Esse material não foi elaborado, e por tanto, não possui PDF.' : ''}
            </Accordion.Header>
          </Accordion.Item>
