...
const ModalPretetion: React.FC<ModalPretetionProps> = ({ show, onClose }) => {
  ...
  const handleSubmit = async () => {
    ...
    try {
      ...
      await api.patch('/plano-alimentar-equivalente-historico/' + planId, payload);
  ...
  return (
    <Modal show={show} onHide={handleClose} backdrop="static" className="modal-close-out">
      <Modal.Header closeButton>
        <Modal.Title>Pretenção de macronutrientes e calorias</Modal.Title>
      </Modal.Header>

      <Modal.Body>
        <Row className="mb-5 row px-3">
          <Col xs="6">
        ...
        <div className="d-flex justify-content-between">
          <div className="mt-1 w-20">
            ...
            <Form.Control className="form-control mt-1 text-center" disabled value={parseFloatNumber(carbohydrates.pretendido_kcal) + ' kcal'} />
            <Form.Control className="form-control mt-1 text-center" disabled value={parseFloatNumber(carbohydrates.pretendido_g) + ' g'} />
            {macrossMode === 'percentage' ? (
              <Form.Control className="form-control mt-1 text-center" disabled value={parseFloatNumber(carbohydrates.pretendido_gkg) + ' g/kg'} />
            ) : (
              <Form.Control className="form-control mt-1 text-center" disabled value={parseFloatNumber(vrCarboidratos).toFixed(1) + '%'} />
            )}
          </div>
          ...
          <div className="mt-1 w-30">
            ...
            <Form.Control className="form-control mt-1 text-center" disabled value={parseFloatNumber(proteins.pretendido_kcal) + ' kcal'} />
            <Form.Control className="form-control mt-1 text-center" disabled value={parseFloatNumber(proteins.pretendido_g) + ' g'} />
            {macrossMode === 'percentage' ? (
              <Form.Control className="form-control mt-1 text-center" disabled value={parseFloatNumber(proteins.pretendido_gkg) + ' g/kg'} />
            ) : (
              <Form.Control className="form-control mt-1 text-center" disabled value={parseFloatNumber(vrProteinas) + '%'} />
            )}
          </div>
          ...
          <div className="mt-1 w-30">
            ...
            <Form.Control className="form-control mt-1 text-center" disabled value={parseFloatNumber(lipids.pretendido_kcal) + ' kcal'} />
            <Form.Control className="form-control mt-1 text-center" disabled value={parseFloatNumber(lipids.pretendido_g) + ' g'} />
            {macrossMode === 'percentage' ? (
              <Form.Control className="form-control mt-1 text-center" disabled value={parseFloatNumber(lipids.pretendido_gkg) + ' g/kg'} />
            ) : (
              <Form.Control className="form-control mt-1 text-center" disabled value={parseFloatNumber(vrLipideos) + '%'} />
            )}
          </div>
        </div>
