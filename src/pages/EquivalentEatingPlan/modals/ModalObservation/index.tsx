...
import { EquivalentEatingPlanOrientation } from '../../../../types/PlanoAlimentarEquivalente';
...
const ModalOrientation: React.FC<ModalOrientationProps> = (props: ModalOrientationProps) => {
  ...
  const { setOrientations } = useEquivalentEatingPlanStore();
  ...
  const onSubmit = async (values: FormValues) => {
    ...
    try {
      const payload: EquivalentEatingPlanOrientation = {
      ...
      if (!orientations[0]) {
        const response = await api.post('/plano-alimentar-equivalente-orientacao/', payload);
        setOrientations([response.data]);
      } else {
        await api.patch('/plano-alimentar-equivalente-orientacao/' + orientations[0].id, payload);
        setOrientations([payload]);
      }
  ...
  const handleRemoveOrientation = async () => {
    if (orientations[0] && orientations[0].id) await api.delete('/plano-alimentar-equivalente-orientacao/' + orientations[0].id);
  ...
  const handleDownloadPdf = async () => {
    ...
    try {
      const { data } = await api.get(
        '/plano-alimentar-equivalente-pdf/' + planId + '/orientacao',
        {
