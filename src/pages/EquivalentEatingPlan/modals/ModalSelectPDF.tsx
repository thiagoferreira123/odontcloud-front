...
import { ClassicEatingPdfLayoutOption } from '../../ClassicEatingPlan/modals/ModalSelectPDF';
...
const ModalSelectPDF = () => {
  ...
  const [buildReplacementList, setBuildReplacementList] = useState(false);
  ...
  const onSubmit = async () => {
    ...
    try {
      ...
      const { data } = await api.post('/plano-alimentar-equivalente-pdf/' + planId, payload, {
      ...
      downloadPDF(data, 'plano-alimentar-equivalente-' + planId);

      updateNotify(toastId.current, 'Pdf gerado com sucesso!', 'Sucesso', 'check', 'success');

      if (buildReplacementList) {
        toastId.current = notify('Gerando pdf da lista de substituição, por favor aguarde...', 'Sucesso', 'check', 'success', true);

        const { data } = await api.post('/plano-alimentar-equivalente-pdf/' + planId + '/lista-substituicao', payload, {
          responseType: 'arraybuffer',
          headers: {
            'Content-Type': 'application/json',
            Accept: 'application/pdf',
        ...
        downloadPDF(data, 'lista-de-substituicao-' + planId);
      ...
      if (buildCommentList) {
        ...
        const { data } = await api.post('/plano-alimentar-equivalente-pdf/' + planId + '/comentarios', payload, {
          responseType: 'arraybuffer',
          headers: {
            'Content-Type': 'application/json',
            Accept: 'application/pdf',
      ...
      if (buildOrientation) {
        ...
        const { data } = await api.get('/plano-alimentar-equivalente-pdf/' + planId + '/orientacao', {
          responseType: 'arraybuffer',
          headers: {
            'Content-Type': 'application/json',
            Accept: 'application/pdf',
  ...
  return (
    <Modal show={showModalSelectPDF} onHide={() => setShowModalSelectPDF(false)} backdrop="static" size="lg" className="modal-close-out">
      ...
      <Modal.Body>
        ...
        <Row>
          <Col md={4}>
            <label className="mb-3 mt-4 ont-bold">Exibição das quantidades</label>
            <div>
              <Form.Check
                type="radio"
                value={1}
          ...
          <Col md={2}>
            ...
            <div>
              <Form.Check
                type="checkbox"
                value={1}
                onChange={() => setBuildReplacementList(!buildReplacementList)}
                label="Lista de substituição"
                id="show_assinatura"
                name="show_assinatura"
                checked={buildReplacementList}
              />
              <Form.Check
                type="checkbox"
                value={1}
