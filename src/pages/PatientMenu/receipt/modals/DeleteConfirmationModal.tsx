...
const DeleteConfirmationModal = () => {
  ...
  const selectedReceipt = useDeleteConfirmationModalStore((state) => state.selectedReceipt);
  const { removeReceipt } = useReceiptStore();
  ...
  const onSubmit = async () => {
    ...
    try {
      if (!selectedReceipt) throw new Error('selectedReceipt is not defined');

      const response = await removeReceipt(selectedReceipt, queryClient);

      if (response === false) throw new Error('Erro ao remover recibo');
  ...
  return (
    <Modal show={showModal} onHide={hideModal}>
      ...
      <Modal.Body>
        Você realmente deseja excluir o recibo? Se sim, digite 'excluir'. Atenção: esta ação é irreversível.
