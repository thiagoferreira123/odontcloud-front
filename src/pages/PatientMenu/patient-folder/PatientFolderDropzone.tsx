...
export default function PatientFolderDropzone() {
  ...
  const getUploadParams = () => ({ url: apiUrl + `/pasta-do-paciente/${id}/upload` });
  ...
  const onChangeStatus = (file: IFileWithMeta, status: StatusValue) => {
    if (status === 'done') {
      ...
      uploadedFiles.find((file) => file.file.id === response[0].id) && removePatientFile(response[0], queryClient);
