...
const DeleteConfirmationModal = () => {
  ...
  const selectedPatientFile = useDeleteConfirmationModalStore((state) => state.selectedPatientFile);
  ...
  const onSubmit = async () => {
    ...
    try {
      if (!selectedPatientFile) throw new Error('selectedPatientFile is not defined');

      const response = await removePatientFile(selectedPatientFile, queryClient);

      if (response === false) throw new Error('Erro ao remover arquivo');
  ...
  return (
    <Modal show={showModal} onHide={hideModal}>
      ...
      <Modal.Body>
        Você realmente deseja excluir o arquivo? Se sim, digite 'excluir'. Atenção: esta ação é irreversível.
