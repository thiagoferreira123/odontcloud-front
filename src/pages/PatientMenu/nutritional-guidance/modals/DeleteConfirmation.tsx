...
import useNutritionalGuidanceSelectedPatientStore from '../hooks';

const DeleteConfirmation = () => {
  const showModal = useDeleteConfirmationStore((state) => state.showModal);

  const selectedNutritionalGuidanceSelectedPatient = useDeleteConfirmationStore((state) => state.selectedNutritionalGuidanceSelectedPatient);
  const { removeNutritionalGuidanceSelectedPatient } = useNutritionalGuidanceSelectedPatientStore();
  ...
  const onSubmit = async () => {
    ...
    try {
      if (!selectedNutritionalGuidanceSelectedPatient) throw new Error('selectedNutritionalGuidanceSelectedPatient is not defined');

      const response = await removeNutritionalGuidanceSelectedPatient(selectedNutritionalGuidanceSelectedPatient, queryClient);

      if (response === false) throw new Error('Erro ao remover orientação');
