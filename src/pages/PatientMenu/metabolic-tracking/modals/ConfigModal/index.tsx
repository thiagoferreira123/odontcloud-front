...
import FormConfigMetabolicTracking from './FormConfigMetabolicTracking';

interface FormConfigMetabolicTrackingRef {
  handleSubmit: () => void;
}

const ConfigModal = () => {
  const formRef = useRef<FormConfigMetabolicTrackingRef>();
  ...
  return (
    <Modal className="modal-right large" show={showModal} onHide={hideConfigModal}>
      <Modal.Header closeButton>
        <Modal.Title>Configurações do rastreamento metabólico</Modal.Title>
      </Modal.Header>
      <Modal.Body>
        <FormConfigMetabolicTracking ref={formRef} setIsLoading={setIsLoading} handleCloseModal={hideConfigModal}/>
      </Modal.Body>
      <Modal.Footer>
        {isLoading ? (
