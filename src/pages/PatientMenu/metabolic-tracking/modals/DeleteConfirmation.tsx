...
const DeleteConfirmation = () => {
  ...
  const selectedMetabolicTracking = useDeleteConfirmationStore((state) => state.selectedMetabolicTracking);
  const { removeMetabolicTracking } = useMetabolicTrackingStore();
  ...
  const onSubmit = async () => {
    ...
    try {
      if (!selectedMetabolicTracking) throw new Error('selectedMetabolicTracking is not defined');

      const response = await removeMetabolicTracking(selectedMetabolicTracking, queryClient);

      if (response === false) throw new Error('Erro ao remover rastreamento metabólico');
  ...
  return (
    <Modal show={showModal} onHide={hideModal}>
      ...
      <Modal.Body>
        Você realmente deseja excluir o rastreamento metabólico? Se sim, digite 'excluir'. Atenção: esta ação é irreversível.
