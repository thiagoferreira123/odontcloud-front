...
const DeleteConfirmation = () => {
  ...
  const selectedExpenditure = useDeleteConfirmationStore((state) => state.selectedExpenditure);
  const { removeExpediture } = useCaloricExpenditureStore();
  ...
  const onSubmit = async () => {
    ...
    try {
      if (!selectedExpenditure) throw new Error('selectedExpenditure is not defined');

      const response = await removeExpediture(selectedExpenditure, queryClient);

      if (response === false) throw new Error('Erro ao remover predição de gasto calórico');

      resetForm();
      setIsLoading(false);
      notify('Predição de gasto calórico removida com sucesso', 'Sucesso', 'bin');
    } catch (error) {
      setIsLoading(false);
      console.error(error);
      notify('Erro ao remover predição de gasto calórico', 'Erro', 'error-hexagon');
    }
  ...
  return (
    <Modal show={showModal} onHide={hideModal}>
      ...
      <Modal.Body>
        Você realmente deseja excluir a predição de gasto calórico? Se sim, digite 'excluir'. Atenção: esta ação é irreversível.
