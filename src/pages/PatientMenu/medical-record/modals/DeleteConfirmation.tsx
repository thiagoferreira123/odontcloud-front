...
const DeleteConfirmationModal = () => {
  ...
  const selectedMedicalRecord = useDeleteConfirmationModalStore((state) => state.selectedMedicalRecord);
  ...
  const onSubmit = async () => {
    ...
    try {
      if (!selectedMedicalRecord) throw new Error('selectedMedicalRecord is not defined');

      const response = await removeMedicalRecord(selectedMedicalRecord, queryClient);

      if (response === false) throw new Error('Erro ao remover prontuário');
  ...
  return (
    <Modal show={showModal} onHide={hideModal}>
      ...
      <Modal.Body>
        Você realmente deseja excluir o prontuário? Se sim, digite 'excluir'. Atenção: esta ação é irreversível.
