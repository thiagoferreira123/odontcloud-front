...
const DeleteConfirmationModal = () => {
  const selectedMeal = useDeleteQualitativeMealConfirmationModal((state) => state.selectedMeal);
  const showModal = useDeleteQualitativeMealConfirmationModal((state) => state.showModal);

  const queryClient = useQueryClient();

  const [isLoading, setIsLoading] = useState(false);

  const { removeQualitativeMeal } = useMyQualitativeMealsStore();
  const { closeModal } = useDeleteQualitativeMealConfirmationModal();
  ...
  const onSubmit = async () => {
    ...
    try {
      ...
      await removeQualitativeMeal(selectedMeal, queryClient);
