...
import { AntropometricAssessmentHistory, BioimpedanceAntropometricData } from '../../../types/AntropometricAssessment';
...
interface AssessmentsChartProps {
  assessments: Array<AntropometricAssessmentHistory<BioimpedanceAntropometricData> | null>;
}
    ...
    return [
      {
        ...
        data: props.assessments.map((assessment) => (assessment && assessment.data ? parseFloatNumber(assessment.data.pesoBioimpedancia ?? 0) : 0)),
      },
      {
        ...
        borderWidth: 1.5,
        borderRadius: parseInt(themeValues.borderRadiusMd, 10),
        data: props.assessments.map((assessment) => (assessment && assessment.data ? parseFloatNumber(assessment.data.massaGordaBioimpedancia ?? 0) : 0)),
      },
      {
        ...
        borderWidth: 1.5,
        borderRadius: parseInt(themeValues.borderRadiusMd, 10),
        data: props.assessments.map((assessment) => (assessment && assessment.data ? parseFloatNumber(assessment.data.massaMagraBioimpedancia ?? 0) : 0)),
      },
    ]
  }, [props.assessments, themeValues.borderRadiusMd, themeValues.primary, themeValues.primaryrgb, themeValues.success, themeValues.successrgb, themeValues.warning, themeValues.warningrgb]);
      ...
      options: {
        ...
        scales: {
          y: {
            stacked: true,
            min: 0,
            max: props.assessments.reduce((acc, assessment) => (assessment && assessment.data ? Math.max(acc, parseFloatNumber(assessment.data.pesoBioimpedancia ?? 0)) : acc), 0) + 4,
            grid: {
              display: true,
              lineWidth: 1,
