...
interface AssessmentsChartProps {
  assessments: Array<AntropometricAssessmentHistory<AdultAntropometricData> | null>;
}
...
const AssessmentsChart = (props: AssessmentsChartProps) => {
  ...
  const datasets: ChartDataset<keyof ChartTypeRegistry, number[]>[] = useMemo(() => {
    ...
    const medida_ossea = props.assessments.reduce((acc, assessment) => {
      return assessment && assessment.data && Number(assessment.data.diametro_bicondio_femural) && Number(assessment.data.diametro_estilo_ulnar) ? true : acc;
    }, false);
    ...
    if(medida_ossea) return [
      ...
      },
      {
        ...
        data: props.assessments.map((assessment) =>
          assessment && assessment.data && Number(assessment.data.diametro_bicondio_femural) && Number(assessment.data.diametro_estilo_ulnar)
            ? parseFloatNumber(assessment.data.peso_osseo ?? 0)
            : 0
        ),
      },
      {
        ...
        data: props.assessments.map((assessment) =>
          assessment && assessment.data && Number(assessment.data.diametro_bicondio_femural) && Number(assessment.data.diametro_estilo_ulnar)
            ? parseFloatNumber(assessment.data.peso_residual ?? 0)
            : 0
        ),
      },
      {
        ...
        borderWidth: 1.5,
        borderRadius: parseInt(themeValues.borderRadiusMd, 10),
        data: props.assessments.map((assessment) => (assessment && assessment.data ? parseFloatNumber(assessment.data.peso_gordo ?? 0) : 0)),
      },
      {
        ...
        data: props.assessments.map((assessment) =>
          assessment && assessment.data && Number(assessment.data.diametro_bicondio_femural) && Number(assessment.data.diametro_estilo_ulnar)
            ? parseFloatNumber(assessment.data.peso_magro ?? 0)
            : assessment && assessment.data && assessment.data.massa_livre_de_gordura_kg ? parseFloatNumber(assessment.data.massa_livre_de_gordura_kg ?? 0) : 0
        ),
      },
    ]
    ...
    return [
      ...
      },
      {
        ...
        data: props.assessments.map((assessment) =>
          assessment && assessment.data && !(Number(assessment.data.diametro_bicondio_femural) && Number(assessment.data.diametro_estilo_ulnar))
            ? parseFloatNumber(assessment.data.massa_livre_de_gordura_kg ?? 0)
            : 0
        ),
      },
    ]
