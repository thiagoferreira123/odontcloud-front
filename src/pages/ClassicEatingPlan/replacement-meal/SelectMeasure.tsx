...
const SelectMeasure = (props: { selectedFood: ClassicPlanReplacementMealFood, mealId: number }) => {
  ...
  const { setShowModalCustomMeasurement, setSelectedReplacementMealFood } = useModalsStore();

  const [options, setOptions] = useState<MeasureOption[]>([
    { label: 'Gramas', value: 'Gramas' },
    { label: 'À vontade', value: 'À vontade' },
    { label: 'Medida Personalizada', value: 'Medida Personalizada' },
  ]);
  ...
  const onChangeMeasure = async (option: SingleValue<MeasureOption>) => {
    try {
      ...
      if(option.value === 'Medida Personalizada') {
        setShowModalCustomMeasurement(true);
        setSelectedReplacementMealFood(props.selectedFood, props.mealId);
        return;
      }
