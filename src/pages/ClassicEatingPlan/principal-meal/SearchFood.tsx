...
export const renderInputComponent = (props: object) => <input {...props} className="form-control" placeholder="Busque pelo alimento" />;

export const renderSuggestion = (suggestion: Food) => {
  const { descricaoDoAlimento = '', tabela = '' } = suggestion;
  return (
    <Row className="g-0 position-relative">
      <Col className="d-flex align-items-center">
        <div className="stretched-link body-link">
          {descricaoDoAlimento} <small className="text-muted"> ({(tabela.charAt(0).toUpperCase() + tabela.slice(1).toLowerCase()).replace('_', ' ')})</small>
        </div>
      </Col>
    </Row>
  );
};
export const getFood = (id: number, table: string, foods: Food[]) => {
  const food = foods?.find((food) => food.id === id && food.tabela.toLowerCase() === table.toLowerCase());
  return food;
};
...
const SearchFood = (props: SearchFoodProps) => {
  ...
  const resetFoodName = async (name: string) => {
    if (typeof props.selectedFood.id === 'number') {
      setValueState(name);
  ...
  return (
    <Autosuggest
      ...
      onSuggestionSelected={(e) => resetFoodName(e.currentTarget.textContent || valueState)}
    />
  );
};
